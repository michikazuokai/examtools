...
| タグ       |   列 | 内容                                                         |
| ---------- | ---: | ------------------------------------------------------------ |
| `b_select` |    B | 表示スタイル：`normal` / `inline` / `inline(8)` など  inlineの(x) xのデフォルトは8mm |
| `select`   |    B | 選択肢本文                                                   |
|            |    C | ラベル（任意）                                               |
...
承知いたしました。左側に列名（A列など）、右側に詳細（tagなど）を配置した縦型の表にいたしました。
...
| E列  | ラベル（番号の後につける）                   |
| F列  | (空欄)                                       |
| G列  | ver2用の選択問題の問題番号と対応した解答番号 |

情報源

| A列       | B列 | C列 | D列   | E列 | F列 | G列     |
|-----------|-----|-----|-------|-----|-----|---------|
| answer    | 2   | 2   | (1,1) | a   | 分類 | B版用正解 |
| subanswer | 3   | 2   | (1,1) | ①   | 分類 | B版用正解 |
...
問題用紙をワード形式のdocxファイルに変換する。
インプット  work/科目番号.json
アウトプット  wordexcel/科目番号_a.docx,科目番号_b.docx

実行方法  make_word.py 科目番号



### 解答用紙の作成

解答用紙をエクセル形式のxlsxファイルに変換する。
インプット  work/科目番号_ans.json
アウトプット  wordexcel/科目番号_a.xlsx,科目番号_b.xlsx

実行方法  make_ansexcel.py 科目番号
