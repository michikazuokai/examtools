...
def _load_validation_stamp(work_dir: Path, subject_no: str) -> dict:
    ...
    if not stamp_path.exists():
        raise RuntimeError(
            "validate_excel.py の実行済みstampが見つかりません。\n"
            "先に次を実行してください。\n"
            f"  python scripts/validate_excel.py {subject_no}\n"
            f"期待したstamp: {stamp_path}"
        )
...
def require_validated_excel(ws, *, work_dir: Path, subject_no: str, sheetname: str, excel_path: Path) -> str:
    ...
    確認内容:
      ...
      - status が ok
      - subject / sheetname が一致する
      - Excelシート内容のhashがstamp内hashと一致する

    戻り値:
      現在のExcel hash
    """
    stamp = _load_validation_stamp(work_dir, subject_no)

    if stamp.get("status") != "ok":
        raise RuntimeError(
            "validation stamp の status が ok ではありません。\n"
            "validate_excel.py でエラーが出ている可能性があります。\n"
            f"stamp status: {stamp.get('status')}"
        )

    if str(stamp.get("subject")) != str(subject_no):
        raise RuntimeError(
            "validation stamp の科目番号が一致しません。\n"
            f"実行科目: {subject_no}\n"
            f"stamp科目: {stamp.get('subject')}"
        )

    if str(stamp.get("sheetname")) != str(sheetname):
        raise RuntimeError(
            "validation stamp のシート名が一致しません。\n"
            f"実行シート: {sheetname}\n"
            f"stampシート: {stamp.get('sheetname')}"
        )

    current_hash = calc_excel_hash(ws)
    stamped_hash = stamp.get("hash")

    if not stamped_hash:
        raise RuntimeError(
            "validation stamp に hash がありません。\n"
            "validate_excel.py を再実行してください。"
        )

    if current_hash != stamped_hash:
        raise RuntimeError(
            "試験問題.xlsx が validate_excel.py 実行後に変更されています。\n"
            "先に validate_excel.py を再実行してください。\n"
            f"Excel hash : {current_hash}\n"
            f"Stamp hash : {stamped_hash}\n"
