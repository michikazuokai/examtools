...
% ---- first-subquestion-question flag ----
\newif\ifFirstSubQ
\FirstSubQfalse

% まずはこの値から（あとで微調整）
\newlength{\MLtoChoiceTightNormal} \setlength{\MLtoChoiceTightNormal}{0mm}
\newlength{\MLtoChoiceTightInline} \setlength{\MLtoChoiceTightInline}{0mm}
...
\newlength{\QBefore} \setlength{\QBefore}{5.0mm}
%%%   \newlength{\QAfter}  \setlength{\QAfter}{1.5mm}
\newlength{\QAfter}  \setlength{\QAfter}{0mm}
...
\newlength{\SubBlockAfter}  \setlength{\SubBlockAfter}{0.0mm}
\newlength{\QBeforeSub}  \setlength{\QBeforeSub}{3mm} % ←小問→次の小問を詰めたいなら0mm
...
\FirstQuestiontrue
% subquestion境界での足し算防止（QBefore/QAfterを1回だけスキップ）
\newif\ifSkipQBeforeOnce
\SkipQBeforeOncefalse
\newif\ifNextIsSubquestion
\NextIsSubquestionfalse

% ---- question env ----
\NewDocumentEnvironment{question}{m}{%
%%%  \par\vspace*{\QBefore}%
%%%  \par\addvspace{\QBefore}%
  \par
  \ifFirstQuestion
    \global\FirstQuestionfalse % 1問目は空けない
    \else
      % subquestion直後など「1回だけQBeforeを入れない」指示があればスキップ
      \ifSkipQBeforeOnce
        \global\SkipQBeforeOncefalse
      \else
        % ページ先頭（自動改ページ/強制改ページの直後）なら空けない
        \ifdim\pagetotal=0pt
          % 何もしない
        \else
          % ★ここだけ分岐：subquestion内は「最初だけ空けない」、2つ目以降はQBeforeSub
          \ifInSubquestion
            \ifFirstSubQ
              \global\FirstSubQfalse % ←最初の小問は空けない
              % 何もしない
            \else
              \addvspace{\QBeforeSub}% ←2つ目以降の小問間の空き
            \fi
          \else
            \addvspace{\QBefore}%
          \fi
  ...
  \begingroup
    ...
    \begin{tabular}[t]{@{}l@{\hspace{\examSepTmp}}p{\dimexpr\linewidth-\Qlabelw-\examSepTmp\relax}@{}}
      \makebox[\Qlabelw][l]{\textbf{\mbox{\examLabelTmp}}} & \strut\textbf{#1}\strut \\
    \end{tabular}%
  \endgroup
  \par

%%%%  \par\vspace*{\QAfter}%
  % questionの直後にsubquestionが来る場合は、ここでは空けない（subquestion側に任せる）
  \ifNextIsSubquestion
    \global\NextIsSubquestionfalse
  \else
    \par\addvspace{\QAfter}%
  \fi
}{%
  \global\AfterMultilinefalse % ★これを追加：次の問題へ持ち越さない
%%%  \par\vspace{0.8\baselineskip}
%%%  \par\addvspace{0.8\baselineskip}
%%%  \par\addvspace{0pt} % Step1: まず余計な足し算を止める
  \par % まずは段落だけ確定（addvspaceは入れない）
}

% ---- subquestion env ----
\NewDocumentEnvironment{subquestion}{}{%
  \global\NextIsSubquestiontrue
  \par\addvspace{\SubBlockBefore}%
  \global\SkipQBeforeOncetrue % ★追加：subquestion直後の最初の小問はQBeforeを入れない
  \begingroup
  \InSubquestiontrue
%%%  \global\FirstSubQtrue
  \begin{list}{}{%
...
%%%  \par\vspace*{\SubBlockAfter}%
  \par\addvspace{\SubBlockAfter}%
  %%%  \global\SkipQBeforeOncetrue
}%
...
\NewDocumentEnvironment{multiline}{O{}}{%
  \pgfkeys{/exam, before=3, after=3, #1}%
...
%    ...
%  \end{premisebox}
% - Used for common preconditions before questions
% ============================================================
\newenvironment{premisebox}{%
  \par
  \Needspace{10\baselineskip}
  % ページ先頭でなければ、前に2行分の余白を入れる
  \ifdim\pagetotal<2\baselineskip
    % ページ先頭に近い場合は余白を入れない
  \else
    \vspace{2\baselineskip}
  \fi
  \hspace*{5mm}\textbf{【前提条件】}\par
  \vspace{1mm}
  \begin{tcolorbox}[
    enhanced,
    left skip=7mm,
    width=\dimexpr\linewidth-7mm\relax,
    colback=white,
    colframe=black,
    boxrule=0.6pt,
    arc=1mm,
    ...
    % --- ここを追加 ---
    valign=center, 
    % もしボックスに固定の高さを指定している場合は以下のように記述します
    % height=3cm, % 例：高さを固定にする場合
  ]
}{%
  \end{tcolorbox}
}
...
%%%  \pgfkeys{/exam/.cd, type=normal, sep=8, vspace=3mm, #1}%
  \pgfkeys{/exam/.cd, type=normal, sep=8, vspace=1mm, #1}%
...
% ---- inline / normal ----
\ifdefstring{\examChoiceType}{inline}{%
  \par\addvspace{0pt}% 段落確定（重なり防止）
  \noindent
  \hspace*{\ChoiceIndentInline}%
  \begin{minipage}[t]{\dimexpr\linewidth-\ChoiceIndentInline\relax}
  \vspace*{0pt}% ★重要：-\topskip を使わない（重なり防止）

  \begin{itemize*}[
    label={},
    leftmargin=0pt,
    afterlabel=\hspace*{\ChoiceLabelSepInline},
    topsep=0pt,
    partopsep=0pt,
    parsep=0pt,
